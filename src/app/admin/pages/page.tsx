 ...
 * 管理后台单页管理页。
 *
 * 单页内容用于承载 about 这类固定入口，数量通常不多；
 * 列表页重点是快速识别 slug 与最近更新时间，而不是复杂筛选。
 */
  ...
  return (
    <div className="space-y-6">
      <PageIntro
        eyebrow="Pages"
        title="单页管理"
        description="这里用于维护 about 这类固定内容入口。与文章不同，单页更强调稳定路由和长期可维护性。"
        aside={
          <div className="rounded-[1.75rem] border border-border bg-primary px-6 py-6 text-primary-foreground">
            ...
            <p className="mt-5 text-sm leading-7 text-primary-foreground/80">
              推荐把关于、使用说明等低频更新内容统一归入单页，而不是塞进站点配置。
            </p>
          </div>
        }
      />
      ...
      <AdminResourceListState loading={loading} empty={pages.length === 0} emptyText="暂无单页">
        <section className="page-frame overflow-hidden">
          <div className="grid gap-4 border-b border-border bg-muted/40 px-5 py-4 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground md:grid-cols-[minmax(0,2fr)_10rem_10rem_8rem]">
          ...
          <div className="divide-y divide-border">
            {pages.map((page) => (
              <div
                key={page.id}
                className="grid gap-4 px-5 py-5 md:grid-cols-[minmax(0,2fr)_10rem_10rem_8rem] md:items-center"
              >
                <div className="min-w-0">
