...
export const getPublicCategoryBySlug = unstable_cache(
  ...
  { revalidate: PUBLIC_CONTENT_REVALIDATE_SECONDS, tags: ['public-taxonomies'] },
);
 ...
 */
export const getPublicTagBySlug = unstable_cache(
  async (slug: string) => {
 ...
 */
export const getPublicTagsWithPostCount = unstable_cache(
  async () => {
 ...
 */
export const getPublicCategorySlugs = unstable_cache(
  async () => {
  ...
  { revalidate: PUBLIC_METADATA_REVALIDATE_SECONDS, tags: ['public-taxonomies'] },
);

/**
 * 获取 sitemap 使用的公开标签 slug 列表。
 */
export const getPublicTagSlugs = unstable_cache(
  async () => {
    return prisma.tag.findMany({
      select: { slug: true },
    });
