...
- 将公开页面从长期 `force-dynamic` 调整为短周期 ISR 缓存，RSS 与 sitemap 使用较长缓存
- 搜索结果在相关度排序后增加发布时间倒序兜底，减少同分结果顺序漂移
- AI 全文优化提示词增加轻量文章类型 profile，先按内容关键词区分通用、技术文章和随笔复盘
- AI 模型输出增加 JSON 对象 schema 校验，避免异常结构进入归一化流程
- 管理端登录接入内存限流和失败审计日志，保持 NextAuth 协议响应不变
- 引入 Redis 运行时依赖，限流和浏览量 UV 去重改为 Redis 存储，compose 默认使用宿主机 6380 端口
